...
import {ContextUpgradeable} from
    ...
    interface IGeoGuardNFT {
        function getNodeId(address owner, uint256 id) external view returns (string memory);
    }
    ...
    ReentrancyGuardUpgradeable
{
    ...
    // Address of the GeoGuard NFT contract
    address constant GEO_GUARD_NFT_ADDRESS = 0x18A87471c520008F3187947Efef644c7E66091b4;

    function checkGeoGuardNFT(nodeId) internal view {
        // Create an instance of the external contract
        IGeoGuardNFT geoGuardNFT = IGeoGuardNFT(GEO_GUARD_NFT_ADDRESS);

        // Call the balanceOf function
        string nodeIdStored = geoGuardNFT.getNodeId(msg.sender, 1);

        // Revert if the balance is less than 4
        if (nodeId != nodeIdStored) {
            revert("GeoGuard NFT not present");
        }
    }
    ...
    function _initiateValidatorRegistration(
        ...
        checkGeoGuardNFT(initiateValidatorRegistration.nodeID);
        if (
